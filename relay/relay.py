...
# Pool is a self-contained module (extracted 2026-08-31). These names
# are re-exported for backward compatibility — the test surface and all
# existing call sites reference them via relay.relay.
#
# Direct-script execution support: the live systemd unit and smoke test run
# `python relay/relay.py`. When __package__ is unset (or empty), Python puts
# the SCRIPT DIR (<repo>/relay/) on sys.path, not the repo root — so the
# `relay` package (relay.pool, relay.config) would not resolve. Prepend the
# repo root so `relay.*` imports work identically to package-import runs.
...
# Point the extracted pool module at this module's LIVE globals so its
# call-time config reads (LATENCY_SKIP_THRESHOLD_MS, cooldown constants)
# honor monkeypatching of relay module globals — the test contract.
#
# Wire in BOTH normal package import (__name__ == "relay.relay") and genuine
# direct run (`python relay/relay.py`, __name__ == "__main__"). The one case
# we must NOT wire is a throwaway re-execution: tests use
# runpy.run_path(relay.py, run_name="__main__") to exercise main(), which
# re-runs this file in a fresh namespace while the real relay.relay is already
# imported and wired — clobbering pool._relay_globals with that throwaway
# namespace would break live monkeypatch reads. Detect it via sys.modules:
# if we're "__main__" but a distinct real relay.relay already exists, skip.
_real_relay = sys.modules.get("relay.relay")
_is_throwaway = __name__ == "__main__" and _real_relay is not None and _real_relay is not sys.modules.get("__main__")
if not _is_throwaway:
    _set_relay_globals(globals())
    # Same live-globals seam for the extracted AuthSwitcher (reads/writes
...
# AuthSwitcher is a self-contained module (extracted 2026-08-31).
# Re-export for backward compatibility — the test surface imports it via relay.relay.
from relay.auth_switcher import AuthSwitcher  # noqa: E402  (mid-file re-export, intentional)
...
from relay.routes_direct import router as _direct_router  # noqa: E402

app.include_router(_health_router)
app.include_router(_v1_router)
app.include_router(_admin_router)
# Direct-egress router: mounted ONLY when DIRECT_EGRESS=true (opencode-zen-normal).
# Serves /v1n/: same free-set filter + shared model cache, but no SOCKS5 pool.
if DIRECT_EGRESS:
    app.include_router(_direct_router)
    logger.info("DIRECT_EGRESS enabled — /v1n direct-egress routes mounted (no pool)")
