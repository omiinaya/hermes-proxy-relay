...
[![Tests](https://img.shields.io/badge/tests-697%20passing-green.svg)](#test-status)
...
- **100% line coverage** — relay, plugin, and MCP fully tested (697 tests).
...
> **Single source of truth.** All configuration is parsed exactly once in
> `relay/config.py` (`config.load/reload/snapshot`). The module-level
> `UPPER_CASE` names you see throughout the code are thin bindings derived
> from that snapshot — import, `--config`, and `/admin/reload-config` all
> re-bind through the same path, so there is no parallel config parse left
> to drift. `tests/test_config_drift.py` permanently guards this.
...
# 697 tests pass (100% coverage): resilience (incl. prod-parity ports + dynamic cap), mock-upstream, cooldown-pool, advanced, remaining, edges, e2e, utils, plugin, package, config-drift
```
